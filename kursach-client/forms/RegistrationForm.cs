...
namespace kursach_client.forms
{
    public partial class RegistrationForm : Form
    {
        ...
        Dictionary <String, string> roleList = new Dictionary<String, string>() { {"Сотрудник","WORKER"},{"Администратор","ADMIN"}};
        ...
        private async Task RegistrIn()
        {
            ...
            var role = roleList[RoleBox.Text];
            ...
            if (!(string.IsNullOrEmpty(login) || string.IsNullOrEmpty(password)|| string.IsNullOrEmpty(FIO)|| string.IsNullOrEmpty(comfirmPassword) || string.IsNullOrEmpty(position) || string.IsNullOrEmpty(role)))
            {
                ...
                if (password.Equals(comfirmPassword))
                {
                    ...
                    if (await result)
                    {
                        MessageBox.Show("Пользователь - " + FIO + " создан", "Пользователь создан", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else {
